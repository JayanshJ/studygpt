...
const sizes: Record<Size, string> = {
  sm: "h-8 w-8 rounded-[3px]",
  md: "h-9 w-9 rounded-[3px]",
  lg: "h-10 w-10 rounded-[3px]",
};
  ...
  ({ className, variant = "ghost", size = "md", label, asChild, ...props }, ref) => {
    ...
    return (
      <Comp
        ...
        title={label}
        className={cn(
          "inline-flex items-center justify-center leading-none transition-[background-color,color,border-color] duration-fast ease-out outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-paper disabled:pointer-events-none disabled:opacity-40",
          variants[variant],
          sizes[size],
          className,
        )}
