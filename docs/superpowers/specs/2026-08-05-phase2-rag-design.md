...
- `getModelConfig()` returns `embeddingModel` (settings value or the `NOMIC`/`OLLAMA_EMBEDDING_MODEL` env default or `nomic-embed-text`).
...
5. Persist the used sources (array of `{materialId, title, snippet, ordinal}`) as JSON in a new nullable `sources` TEXT column on the **assistant** message, written in `onFinish` alongside the content upsert.
6. The client renders a Sources panel under that assistant message. **Transport:** `sources` is included in the conversation GET's message rows (so reload shows it); after a live stream the client does one targeted `GET /api/messages/[id]` to fetch sources for the just-finished assistant message and renders the panel immediately.
...
components/
  ProjectsView.tsx               # new (or MaterialsManager.tsx)
...
- **ID pass-through + idempotent inserts** continue (materials/chunks use server UUIDs; the assistant `sources` column is written via the existing `upsertMessage`/`onFinish` path so stop/regenerate semantics stay consistent).
- **One commit per task.**