...
Rendering: emit a single \`mermaid\` fenced block IF Mermaid can faithfully express this notation. If it CANNOT (for example Chen-style ER with rectangles for entities, diamonds for relationships, ovals for attributes, and labelled cardinalities on the edges — Mermaid's erDiagram is crow's-foot only), emit a single \`artifact\` fenced block containing a complete, self-contained HTML document with hand-drawn inline SVG that reproduces THIS notation — same shapes, same connectors, same labels, same layout conventions as the note above. This override takes precedence over any general "use mermaid for diagrams" instruction.`;
}